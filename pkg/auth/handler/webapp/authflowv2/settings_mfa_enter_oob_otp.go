...
func NewSettingsMFAEnterOOBOTPViewModel(channel model.AuthenticatorOOBChannel, target string, now time.Time, state *otp.State) SettingsMFAEnterOOBOTPViewModel {
	...
	case model.AuthenticatorOOBChannelSMS:
		maskedClaimValue = phone.Mask(target)
	case model.AuthenticatorOOBChannelEmail:
		maskedClaimValue = mail.MaskAddress(target)
	}
...
func (h *AuthflowV2SettingsMFAEnterOOBOTPHandler) GetData(r *http.Request, w http.ResponseWriter, channel model.AuthenticatorOOBChannel, target string) (map[string]interface{}, error) {
	now := h.Clock.NowUTC()
	...
	screenViewModel := NewSettingsMFAEnterOOBOTPViewModel(channel, target, now, state)
...
func (h *AuthflowV2SettingsMFAEnterOOBOTPHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	ctrl.Get(func() error {
		...
		data, err := h.GetData(r, w, token.Authenticator.OOBOTPChannel, token.Authenticator.OOBOTPTarget)
		if err != nil {
			return err
		}
