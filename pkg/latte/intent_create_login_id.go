...
func (i *IntentCreateLoginID) ReactTo(ctx context.Context, deps *workflow.Dependencies, workflows workflow.Workflows, input workflow.Input) (*workflow.Node, error) {
	...
	case 0:
		...
		} else if workflow.AsInput(input, &inputTakeProofOfPhoneNumberVerification) && i.LoginIDType == model.LoginIDKeyTypePhone {
			spec := &identity.Spec{
				Type: model.IdentityTypeLoginID,
				LoginID: &identity.LoginIDSpec{
					Type:  i.LoginIDType,
					Key:   i.LoginIDKey,
					Value: stringutil.NewUserInputString(i.PhoneNumberHint),
				},
			}
